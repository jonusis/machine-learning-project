    ...
    return (
      <div className="FirstPage">
        ...
        <div className="feature">
          ...
          <div>因此，本项目旨在： </div>
          <div>+ 搭建网络不良语言信息监测仿真平台，建立完善的相关API接口调用，为建设网络环境不良文本检测平台提供一套可行的方案； </div>
          <div>+ 研究基于半监督学习的网络不良语言自动监测方法，迅速，有效的实现对于网络长文本短文本中存在的不良语言信息进行自动学习和识别。 </div>
          <div>+ 构建不良语言词典和网络不良语料库，为网络不良语言信息的监测和应用技术提供支持。</div>
          <div>+ 克服现有条件对网络舆论检测的局限，迅速有效的实现对网络中不良语言的自动学习与识别。在保证准确率的同时降低人工检测成本，并可识别由于缺乏先验知识、难以人工标注的不良语言。 </div>
          <div>+ 经过半监督学习，构建科学合理且可动态扩展的不良语言语料库，实现不良语言的动态监测和统计分析，为网络不良语言的管理提供一些参考与帮助。 本网站为服务该项目搭建的平台。同时对于最新机器学习相关新闻与分享进行二次组织与深度聚合，从而有效提升信息服务质量，服务于学习者的信息获取需求； 网站首期提供的分类器包括：Centroid算法，FastText，TextCNN，bert等，同时创造性的使用基于图结构的KNN算法；涉及半监督算法有：简单自训练，MixMatch等。 </div>
